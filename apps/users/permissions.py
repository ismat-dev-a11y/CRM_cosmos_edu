...
class IsMentorOfStudent(BasePermission):
  def has_object_permission(self, request, view, obj):
    return (
      request.user.role == "MENTOR" and obj.mentor == request.user
    )
...
class CanBeStudent(BasePermission):
  def has_permission(self, request, view):
    return (request.user.is_authenticated and request.user.role == 'STUDENT' and request.method in ('GET', 'HEAD', 'OPTIONS'))
