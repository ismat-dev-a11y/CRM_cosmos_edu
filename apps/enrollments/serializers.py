...
class EnrollmentSerializers(serializers.ModelSerializer):
  ...
  class Meta:
    model = Enrollment
    fields = ['id','student', 'student_name', 'course', 'course_title', 'enrolled_at', 'is_active', 'is_paid']
  ...
  def validate(self, attrs):
    ...
    if Enrollment.objects.filter(student=student, course=course).exists():
      raise serializers.ValidationError({'error': 'Siz bu kursga allaqachon yozilgansiz!'})
...
class AttendanceSerializer(serializers.ModelSerializer):
  class Meta:
    model = Attendance
    fields = ['id', 'lesson', 'student', 'status', 'date', 'notes']
