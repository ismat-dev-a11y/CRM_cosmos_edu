...
class GroupStudentSerializers(serializers.ModelSerializer):
  ...
  def validate(self, attrs):
    ...
    if group.students.count() >= group.max_students:
      raise serializers.ValidationError({'error':f'Bu yerda joy qolmadi(Maksimal: {group.max_students})'})
    return attrs
...
class GroupSerializer(serializers.ModelSerializer):
  ...
  class Meta:
    model = Group
    fields = [
      'id', 'name', 'course', 'course_name', 'mentor', 'mentor_name',
      'room', 'room_name', 'lesson_days', 'start_time', 'end_time',
      'start_date', 'status', 'max_students', 'student_count'
    ]
